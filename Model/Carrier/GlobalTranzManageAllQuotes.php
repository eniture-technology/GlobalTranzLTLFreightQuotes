 ...
 */
class GlobalTranzManageAllQuotes
{
        ...
        if ($quotesCount == 1) {
            ...
            if (count($this->smallPackagesQuotes) == 0 || count($this->ltlPackagesQuotes) == 0) {
                ...
                if ($multiShipmentValue == 'all') {
                    ...
                    $resultQuotes = $this->gtGetAllQuotes(false, true);
                    return $this->getQuotesForMultiShipment($resultQuotes, $smallModulesArr, $ltlModulesArr);
                } else {
                    $resultQuotesArr = $this->gtGetAllQuotes(true, false);
    ...
    /**
     * This function returns is quotes have multi-packaging or not
     * @return string
     */
     ...
     * @return array
     */
    public function gtGetAllQuotes($getMinimum = false, $isMultishipment = false)
    {
        ...
        foreach ($this->quotes as $key => $quote) {
            ...
            $ltlResultData = $dataHelper->getQuotesResults($quote, $getMinimum, $isMultishipment, $this->scopeConfig, $this->registry);
            if ($ltlResultData != false && $ltlResultData !== null) {
                $resultArr[$key] = $ltlResultData;
            }
        }
        return $resultArr;
    }
